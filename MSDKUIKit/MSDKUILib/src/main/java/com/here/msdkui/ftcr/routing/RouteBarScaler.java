...
public final class RouteBarScaler {
    ...
    private final Map<FTCRRoute, Float> mRouteBarScale = new ConcurrentHashMap<>();
     ...
     */
    public Map<FTCRRoute, Float> scaleRoutes(FTCRRouteOptions ftcrRouteOptions,
                                             Collection<FTCRRoute> routes) {
        long maxDuration = 0;
        for (final FTCRRoute route : routes) {
            if (ftcrRouteOptions
                    .getTransportMode() != FTCRRouteOptions.TransportMode.PEDESTRIAN) {
    ...
    private long getDurationInMilliSeconds(final FTCRRoute route) {
     ...
     */
    public float getScaling(final FTCRRoute input) {
