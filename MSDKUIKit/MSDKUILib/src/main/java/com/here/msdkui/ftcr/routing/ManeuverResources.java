...
public class ManeuverResources {
    ...
    private final List<FTCRManeuver> mManeuverList;
     ...
     */
    public ManeuverResources(final Context context, final List<FTCRManeuver> maneuverList) {
    ...
    public int getManeuverIconId(final int index) {
        final FTCRManeuver maneuver = getManeuverAt(index);
        if (maneuver == null) {
            return 0;
        }
        if (FTCRUtil.getAction(maneuver) == Maneuver.Action.FERRY
                && isCarShuttleTrainManeuver(maneuver)) {
            return R.drawable.ic_maneuver_icon_motorail;
        }
        return getManeuverIconId(FTCRUtil.getIcon(maneuver));
    }
    ...
    public String getManeuverInstruction(final int index) {
        ...
        if (instruction == null || TextUtils.isEmpty(instruction)) {
            // TODO no maneuver.getMapOrientation() in FTCR
//            instruction = mContext.getString(R.string.msdkui_maneuver_head_to,
//                    getLocalizedOrientation(mContext, 0));
        }
        return instruction;
    }
     ...
     */
    public int getDistanceFromNext(List<FTCRManeuver> maneuvers, final int index) {
        final FTCRManeuver nextManeuver = getManeuverAt(index + 1);
        if (nextManeuver != null) {



            return GuidanceManeuverUtil.getDistanceFromPreviousManeuver(maneuvers, nextManeuver);
        }
        return 0;
    }
     ...
     */
    public String getExitDirections(final FTCRManeuver maneuver) {
        // TODO no signpost API in FTCR
        if (true) return "";

        final Signpost signpost = null;//maneuver.getSignpost();
        if (signpost == null) {
            return null;
        }
    ...
    private String getRoadName(final int index) {
        final FTCRManeuver currentManeuver = getManeuverAt(index);
        if (currentManeuver == null) {
            return "";
        }
        String roadName = !currentManeuver.getRoadNames().isEmpty()
                ? currentManeuver.getRoadNames().get(0) : "";
        String roadNumber = "";//currentManeuver.getRoadNumber();
        if (isManeuverChangingRoad(currentManeuver) || index == 0) {
            roadName = !currentManeuver.getNextRoadNames().isEmpty() ?
                    currentManeuver.getNextRoadNames().get(0) : "";
    ...
    private String determineNextManeuverStreet(final int index) {
        ...
        FTCRManeuver afterNextManeuver = getManeuverAt(++i);
        while (distance < NEXT_NEXT_MANEUVER_THRESHOLD && afterNextManeuver != null
                && nextManeuverStreetValue == null) {
            distance += GuidanceManeuverUtil
                    .getDistanceFromPreviousManeuver(mManeuverList, afterNextManeuver);
            nextManeuverStreetValue = format(!afterNextManeuver.getNextRoadNames().isEmpty() ?
     ...
     */
    private FTCRManeuver getManeuverAt(final int maneuverIndex) {
    ...
    private boolean isManeuverChangingRoad(final FTCRManeuver maneuver) {
        if (maneuver != null) {
            final Maneuver.Action action = FTCRUtil.getAction(maneuver);
     ...
     */
    private String getInstruction(final FTCRManeuver maneuver) {
        final Maneuver.Action action = FTCRUtil.getAction(maneuver);
        String ret;
        switch (action) {
            case CHANGE_HIGHWAY:
            case CONTINUE_HIGHWAY:
                ret = getHighwayInstructions(FTCRUtil.getTurn(maneuver));
                break;
            case FERRY:
            ...
            case JUNCTION:
            case ROUNDABOUT:
                ret = getString(RESOURCE_MAP.get(FTCRUtil.getTurn(maneuver)));
                break;
            default:
     ...
     */
    private boolean isCarShuttleTrainManeuver(final FTCRManeuver maneuver) {
        // TODO no API in FTCR for this
        final List<RoadElement> roadElements = null;//maneuver.getRoadElements();
