 ...
 * date, time and a {@link RouteOptions.TimeType} that could be used
...
public class TravelTimePicker extends DialogFragment {
    ...
    private FTCRRouteOptions.TimeType mTimeType = FTCRRouteOptions.TimeType.DEPARTURE;
    ...
    /**
     * An enum that defines the {@link RouteOptions.TimeType} that
     * can be set using this picker.
     * Default is {@link Variety#DEPARTURE} which will result in
     * {@link RouteOptions.TimeType#DEPARTURE} when a time and date is picked.
     *
     * <p>
    ...
    private void updateDefaultValues() {
        ...
        if (mVariety == Variety.ARRIVAL) {
            mTimeType = FTCRRouteOptions.TimeType.ARRIVAL;
        } else {
            mTimeType = FTCRRouteOptions.TimeType.DEPARTURE;
        }
    }
    ...
    void handleClick() {
        if (mDatePicker.getVisibility() == View.VISIBLE) {
            ...
            mCalendar.set(mDatePicker.getYear(), mDatePicker.getMonth(), mDatePicker.getDayOfMonth(),
                    hour, min);
            mTimeType = FTCRRouteOptions.TimeType.values()[mTabLayout.getSelectedTabPosition()];
     ...
     */
    public void setTimeType(final FTCRRouteOptions.TimeType type) {
        if (type == null) {
    ...
    public interface OnTimePickedListener {
         ...
         */
        void onTimePicked(Date time, FTCRRouteOptions.TimeType type);
    }
}
