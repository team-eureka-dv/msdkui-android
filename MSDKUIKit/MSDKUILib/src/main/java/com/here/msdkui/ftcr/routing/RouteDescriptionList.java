...
public class RouteDescriptionList extends CustomRecyclerView {
    ...
    private final List<FTCRRoute> mRouteList = new ArrayList<>();
    ...
    private void init(final AttributeSet attrs) {
        attrsInit(attrs);
        mAdapter = new RouteDescriptionListAdapter(mRouteList, mRoutePlan);
        setAdapter(mAdapter);
    }
    ...
    public void setRoutes(FTCRRoutePlan plan, final List<FTCRRoute> routes) {
        ...
        this.mRoutePlan = plan;
        mAdapter.setRoutePlan(mRoutePlan);
        sort(routes);
        mRouteList.clear();
    ...
    public void setRoutesResult(FTCRRoutePlan routePlan, final List<FTCRRoute> routesResult) {
        ...
        setRoutes(routePlan, routesResult);
    }
    ...
    private void sort(final List<FTCRRoute> routes) {
        switch (mSortType) {
            case DISTANCE:
    ...
    private void sortByDistance(final List<FTCRRoute> routes) {
        Collections.sort(routes, new Comparator<FTCRRoute>() {
            @Override
            public int compare(FTCRRoute route1, FTCRRoute route2) {
                if (mSortOrder == SortOrder.DESCENDING) {
                    return (int) (route2.getLength() - route1.getLength());
                } else {
    ...
    private void sortByTime(final List<FTCRRoute> routes) {
        Collections.sort(routes, (final FTCRRoute route1, final FTCRRoute route2) -> {
            final int tta1 = (int) route1.getTrafficTime();
            final int tta2 = (int) route2.getTrafficTime();
